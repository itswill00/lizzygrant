...
const photos = [
  { src: '/jeremy/02.jpg', alt: 'Lana and Jeremy — seated at RRL event, Western Wear', cap: 'RRL Western Wear — together at event, 2025' },
  { src: '/jeremy/03.jpg', alt: 'Lana and Jeremy — close, RRL caps', cap: 'RRL — close, same night, his cap forward' },
  { src: '/jeremy/04.jpg', alt: 'Lana and Jeremy — black-white selfie on boat, Ford cap', cap: 'On the boat — black-white selfie, lake behind' },
  { src: '/jeremy/08.jpg', alt: 'Lana and Jeremy — full seated with boots, RRL', cap: 'Seated full — boots, dresses, RRL' },
  { src: '/jeremy/11.jpg', alt: 'Lana and Jeremy — laughing outside, car behind', cap: 'Laughing outside — his RRL cap, her phone in hand' },
  { src: '/jeremy/10.jpg', alt: 'Lana and Jeremy — talking to guests at event', cap: 'Talking to guests — same leather, same night' },
  { src: '/jeremy/05.jpg', alt: 'Lana and Jeremy — bayou, Coke, airboat behind', cap: 'Bayou — Coke on shoulder, airboat behind, 2024' },
  { src: '/jeremy/06.jpg', alt: 'Lana and Jeremy — sitting on dock, evening', cap: 'Dock at dusk — sitting together, Indiana vibes' },
  { src: '/jeremy/13.jpg', alt: 'Lana and Jeremy — dancing in workshop, COSTA shirt', cap: 'Dancing in the bayou workshop — COSTA, denim shorts' },
  { src: '/jeremy/14.jpg', alt: 'Lana and Jeremy — wedding cake Love on the Bayou', cap: 'Cake — “Love on the Bayou” (from fan archive)' },
  { src: '/jeremy/15.jpg', alt: 'Lana and Jeremy — holding hands at altar', cap: 'Hands — altar, Sep 26 2024' },
];
        ...
        <div className="bg-[#1A1A1A] p-0">
          <div className="grid gap-0 md:grid-cols-[1.2fr_0.8fr]">
            <div className="relative aspect-[3/4] overflow-hidden bg-[#2E4057]">
              <SafeImage src={photos[0].src} alt={photos[0].alt} className="h-full w-full object-cover" />
          ...
          {photos.map((p, i) => (
            <motion.figure key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="overflow-hidden rounded-2xl border border-[#D4AF37]/15 bg-white dark:border-[#F7F4EB]/10 dark:bg-noir-soft">
              <div className="aspect-[3/4] overflow-hidden bg-parchment-dark">
                <SafeImage src={p.src} alt={p.alt} loading={i < 3 ? 'eager' : 'lazy'} fetchpriority={i < 3 ? 'high' : undefined} className="h-full w-full object-cover" />
              </div>
