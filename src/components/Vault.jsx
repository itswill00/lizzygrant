    ...
    <section id="vault" className="relative overflow-x-hidden bg-espresso/[0.02] py-8 dark:bg-parchment/[0.02] sm:py-12 md:py-16">
      ...
      <div className="mx-auto max-w-[1400px] px-3 sm:px-4 md:px-6">
        <div className="mb-6 flex flex-col gap-4 sm:mb-8 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
          ...
          <div className="hidden rounded-2xl border border-[#D4AF37]/15 bg-white px-4 py-3 font-sans text-[11px] leading-relaxed tracking-wide text-zinc-500 shadow-[0_4px_12px_rgba(0,0,0,0.04)] dark:border-[#F7F4EB]/10 dark:bg-noir-soft sm:block">
            <div className="font-semibold tracking-[0.08em] text-espresso dark:text-parchment">ACCESS INSTRUCTIONS</div>
            Hover to lift / Tap INSPECT to open<br />
            <span className="text-cherry">Red stamp = rarity</span> / Brass = poetry
          </div>
          <div className="rounded-full border border-dashed border-[#D4AF37]/20 bg-parchment-dark/40 px-3 py-2 font-sans text-[10px] tracking-wide text-zinc-500 shadow-sm dark:border-[#F7F4EB]/10 dark:bg-noir sm:hidden">
          ...
          {vaultItems.map((item, idx) => (
            ...
            >
              <div className="flex items-center justify-between gap-2 border-b border-dashed border-[#D4AF37]/15 bg-parchment-dark/50 px-2.5 py-2 dark:border-[#F7F4EB]/10 dark:bg-noir sm:px-3">
                <span className="truncate font-sans text-[9px] tracking-[0.12em] text-zinc-500 sm:text-[10px] sm:tracking-[0.2em]">
                  {item.catalog} / {item.type}
                </span>
                <span
                  className="shrink-0 rounded-full px-1.5 py-0.5 font-sans text-[8px] font-bold tracking-[0.12em] text-white sm:px-2 sm:text-[9px] sm:tracking-[0.08em]"
              ...
              <div className="relative bg-[#1A1A1A] p-2.5 sm:p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="hidden font-sans text-[9px] tracking-widest text-white/40 sm:block">SIDE A</span>
                </div>
              ...
              <div className="relative aspect-[16/10] w-full overflow-hidden bg-parchment-dark">
                ...
                <div className="absolute bottom-1.5 left-2 rounded-full bg-white/90 px-1.5 py-0.5 font-sans text-[7px] tracking-widest text-espresso shadow sm:bottom-2 sm:left-2">POLAROID / LIVE</div>
              </div>
              ...
              <div className="flex flex-1 flex-col p-3 sm:p-4">
                ...
                <p className="mt-1.5 line-clamp-3 flex-1 font-body text-[14px] leading-[1.75] text-zinc-500 dark:text-parchment/70 sm:mt-2 sm:line-clamp-4 sm:text-[15px]">
                  {item.description}
                </p>
                <div className="mt-3 flex items-center justify-between border-t border-dashed border-[#D4AF37]/15 pt-2.5 dark:border-[#F7F4EB]/10 sm:mt-4 sm:pt-3">
                  <span className="font-sans text-[9px] tracking-[0.12em] text-zinc-500 sm:text-[10px] sm:tracking-[0.08em]">
                  ...
                  <button
                    ...
                    className="rounded-full bg-cherry px-3 py-1 font-sans text-[10px] tracking-[0.12em] text-white shadow-sm transition-all duration-300 hover:bg-cherry-light hover:shadow-md active:scale-95 sm:px-3.5 sm:tracking-[0.08em]"
                  >
                    INSPECT →
                  </button>
                </div>
            ...
            >
              <div className="relative aspect-[16/9] w-full shrink-0 overflow-hidden rounded-t-2xl bg-parchment-dark">
                ...
                <div className="absolute left-3 top-3 flex items-center gap-2">
                  <span className="rounded-full bg-white/90 px-2 py-1 font-sans text-[10px] tracking-[0.12em] text-espresso shadow">{selected.catalog}</span>
                  <span className="rounded-full px-2 py-1 font-sans text-[10px] tracking-[0.12em] text-white shadow" style={{ background: selected.color }}>{selected.stamp}</span>
                </div>
                <button
                  onClick={() => setSelected(null)}
                  className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-black/50 font-sans text-white backdrop-blur hover:bg-black/70"
                  aria-label="Close"
                >
                  ✕
                </button>
                <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5">
                  <div className="font-sans text-[10px] tracking-[0.08em] text-white/80">{selected.type} / {selected.year}</div>
              ...
              <div className="p-4 sm:p-6">
                <p className="font-body text-[15px] leading-[1.75] text-zinc-500 dark:text-parchment/70">{selected.description}</p>
                ...
                <div className="mt-6 flex gap-2">
                  <button onClick={() => setSelected(null)} className="flex-1 rounded-full border border-[#D4AF37]/20 bg-white px-4 py-3 font-sans text-[11px] tracking-[0.14em] text-espresso transition hover:bg-[#F7F4EB] dark:border-[#F7F4EB]/15 dark:bg-noir-soft dark:text-parchment">
                    CLOSE FILE ✕
                  </button>
                  <button
                    onClick={() => {
