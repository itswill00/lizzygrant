      ...
      <div className="mb-5 flex flex-wrap gap-1.5 sm:mb-6 sm:gap-2">
        {galleryEras.map((e) => (
          <button
            ...
            className={`min-h-[36px] rounded-full border px-3.5 py-2 font-sans text-[11px] tracking-[0.1em] transition-all duration-300 active:scale-95 ${
              filter === e.id
                ? 'border-cherry bg-cherry text-white shadow-[0_4px_12px_rgba(158,27,27,0.25)]'
                : 'border-[#D4AF37]/20 bg-white text-zinc-500 hover:bg-[#F7F4EB] dark:border-[#F7F4EB]/10 dark:bg-noir-soft'
            }`}
          >
            {e.label}
          </button>
        ))}
        <span className="ml-auto hidden self-center font-sans text-[10px] tracking-[0.14em] text-zinc-500/60 sm:block">
          ...
          {shown.map((f, i) => (
            ...
            >
              <div className="relative aspect-[3/4] overflow-hidden rounded-xl bg-parchment-dark">
                ...
                />
                <span className="absolute left-1.5 top-1.5 rounded-full bg-black/55 px-1.5 py-0.5 font-sans text-[8px] tracking-[0.12em] text-white backdrop-blur-sm">
      ...
      <div className="mt-6 flex flex-col items-center gap-2">
        ...
        {visible < filtered.length && (
          <button
            ...
            className="min-h-[40px] rounded-full bg-cherry px-6 py-2.5 font-sans text-[11px] tracking-[0.08em] text-white shadow-[0_4px_12px_rgba(158,27,27,0.25)] transition-all duration-300 hover:bg-cherry-light active:scale-95"
          >
            LOAD MORE FRAMES ↓
          </button>
        )}
          ...
          <>
            ...
            >
              <div className="relative overflow-hidden rounded-t-2xl bg-[#1A1A1A]">
                ...
                <button onClick={() => setLightbox((lightbox - 1 + filtered.length) % filtered.length)} aria-label="Previous frame" className="absolute left-3 top-1/2 flex min-h-[44px] min-w-[44px] -translate-y-1/2 items-center justify-center rounded-full bg-black/50 font-sans text-white backdrop-blur hover:bg-black/70 active:scale-95">
                  ←
                </button>
                <button onClick={() => setLightbox((lightbox + 1) % filtered.length)} aria-label="Next frame" className="absolute right-3 top-1/2 flex min-h-[44px] min-w-[44px] -translate-y-1/2 items-center justify-center rounded-full bg-black/50 font-sans text-white backdrop-blur hover:bg-black/70 active:scale-95">
                  →
                </button>
                <span className="absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full bg-black/55 px-3 py-1 font-sans text-[10px] tracking-[0.08em] text-white backdrop-blur-sm">
